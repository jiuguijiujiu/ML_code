...
def dm01_data_split():
    ...
    # print(x.head())
    # print(y.head())
    # print(f'标签列的类别分布情况：{Counter(y)}')

    # 3. 切分数据集
    # 参1：特征数据，参2：标签数据，参3：测试集的比例，参4：随机种子，参5：参考数据集的标签分布.
    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size = 0.2, random_state = 23, stratify = y)

    # 4. 把上述的 训练集特征 和 标签数据拼接到一起，测试集特征 和 标签数据拼接到一起，最后写到文件中。
...
def dm02_train_model():
    ...
    # 6. 模型保存
    # joblib.dump(estimator, './model/xgb_model.pkl')     # 后缀名也可以写 .pth，都是pickle文件格式
...
def dm03_use_model():
    ...
    # 3. 加载模型
    # estimator = joblib.load('./data/xgb_model.pkl')
    estimator = xgb.XGBClassifier()

    # 4. 创建网格搜索 + 交叉验证(结合分层采样数据) 找最优超参数组合
    # 4.1 定义变量,记录参数组合
    ...
    gs_estimator = GridSearchCV(estimator, param_grid=param_grid, cv=skf)
    ...
    # 7. 模型评估
    print(f'最优评估器对象:{gs_estimator.best_estimator_}')
    print(f'最优评分:{gs_estimator.best_score_}')
    print(f'模型评估报告:{classification_report(y_test, y_pred)}')
